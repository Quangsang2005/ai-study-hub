...
function AiChatPage() {
  ...
  useEffect(() => {
    ...
    const aiResponse = {
      id: initialMessageIdRef.current + 1,
      ...createMockAiResponse(initialMessage)
    };
    setMessages(prev => {
      const hasInitialResponse = prev.some(message => message.id === aiResponse.id);
      const nextMessages = hasInitialResponse ? prev : [...prev, aiResponse];
  ...
  const handleSendMessage = (text) => {
    ...
    localThreadMessagesRef.current = [...messages, newMessage];
    setLocalThreads(prev => {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => {
        const nextMessages = [...prev, aiResponse];
        localThreadMessagesRef.current = nextMessages;
        return nextMessages;
      });
    }, 1000);
  ...
  return (
    <div style={{ display: 'flex', margin: '-14px -28px -32px', height: 'calc(100vh - 70px)', background: '#fafafa' }}>
      <ChatSidebar
